...
data = data %>% select(year, county, n1, per_income, per_wage, unemployed, 
                       edu, old, young, lfp, black, hispanic, pop, m_share); unique(data$county)
ar = data %>% group_by(year) %>% summarise(per_wage = mean(per_wage)); ar
...
writeLines(htmlTable(county_control, rnames = F), "control_county.html")
...
p4 = gc %>% plot_custom(title = 'Garland County') + AR +
...
p5 = jc %>% plot_custom(title = 'Jefferson County') + AR +
...
p6 = cc %>% plot_custom(title = 'Crittenden County') + AR +
...
p = p4/p5/p6 ; ggsave(plot = p, filename = 'figure2.png', width = 10, height=13)
...
writeLines(htmlTable(county_table, rnames = F), "predictors_county.html")
