...
data = data %>% filter(city %in% top100$city | city %in% c('Hot Springs', 'Pine Bluff', 'West Memphis')) %>% 
  select(year, city, job, emp, per_wage, wages_n, unemployed, edu, old, young, lfp, black, hispanic, pop); unique(data$city)
data %>% filter(year==2018) %>% arrange(wages_n) %>% select(city, wages_n) 
data %>% filter(city=='Hot Springs')
data %>% filter(city=='Pine Bluff')
city_sc = function(treated, exclude, time, period) {
  data %>% filter(!(city %in% exclude)) %>% 
    ...
    generate_weights(optimization_window = period, sigf_ipop = 3) %>% # 6 won't work for west memphis
    generate_control()
}
hs2 = city_sc('Hot Springs', c('Pine Bluff', 'West Memphis'), 2019, 2011:2018)
pb2 = city_sc('Pine Bluff', c('West Memphis', 'Hot Springs'), 2020, 2011:2019)
wm2 = city_sc('West Memphis', c('Pine Bluff', 'Hot Springs'), 2019, 2011:2018)
...
(hs_control = control(hs2) %>% mutate(x = paste0(unit, ' (', round(weight, 3), ')')) %>% pull(x))
(pb_control = control(pb2) %>% mutate(y = paste0(unit, ' (', round(weight, 3), ')')) %>% pull(y))
(wm_control = control(wm2) %>% mutate(z = paste0(unit, ' (', round(weight, 3), ')')) %>% pull(z))
...
writeLines(htmlTable(city_control, rnames = F), "control_city_job.html")
...
p1 = hs2 %>% plot_custom(title = 'Hot Springs') + AR +
...
p2 = pb2 %>% plot_custom(title = 'Pine Bluff') + AR +
...
p3 = wm2 %>% plot_custom(title = 'West Memphis') + AR +
...
p = p1/p2/p3; ggsave(plot = p, filename = 'figure3.png', width = 10, height=13)

## predictors summary stats
library(forcats)
hs_sum = hs2 %>% grab_balance_table() %>% select(-donor_sample)
pb_sum = pb2 %>% grab_balance_table() %>% select(-donor_sample)
...
summary_table[1:7,-1] = lapply(summary_table[1:7,-1], function(x) round(x, 1))
summary_table[8:10,-1] = lapply(summary_table[8:10,-1], function(x) round(x, 3))
(city_table = summary_table)
writeLines(htmlTable(city_table, rnames = F), "predictors_city_job.html")
